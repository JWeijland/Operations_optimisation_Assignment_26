...
\begin{abstract}
This project optimizes aircraft landing operations using a Mixed-Integer Linear Programming (MILP) model. The goal is to minimize costs from deviations from preferred landing times while meeting all safety separation requirements across multiple runways.

The MILP model is based on existing research and tested using computer-generated data that represents a realistic Schiphol airport scenario with three aircraft types and different separation needs. We use synthetic data instead of real airport data because real arrival times are already optimized by air traffic controllers, making them unsuitable for testing optimization algorithms. The analysis covers up to 3 runways, as computational time becomes very large beyond this limit.

The report verifies that the model works correctly and analyzes how key parameters affect performance, including the number of runways, number of aircraft, and penalty costs. Results show that the MILP approach provides major improvements over simple methods, achieving cost reductions up to 40\% while remaining computationally practical for realistic problem sizes.
\end{abstract}
...
The aircraft landing scheduling problem is one of the most important optimization challenges in modern air traffic management. As global air traffic continues to grow, airports must maximize runway capacity while maintaining strict safety standards. The problem involves determining the best landing times and runway assignments for arriving aircraft while respecting minimum separation requirements and minimizing costs from schedule deviations.

This problem is complex because of multiple competing goals and constraints. Aircraft have preferred landing times based on their flight schedules, and deviations from these targets create costs for airlines and passengers. However, safety rules require minimum time separations between consecutive landings based on aircraft types and wake turbulence categories. When multiple runways are available, the optimization must decide both the landing sequence and runway assignment for each aircraft.

This project implements a Mixed-Integer Linear Programming model for the aircraft landing scheduling problem. The model is based on established research but applied to a realistic Schiphol-inspired scenario. An important methodological choice is using computer-generated arrival data rather than real operational data. This choice is justified because actual arrival times at operational airports have already been optimized by air traffic controllers, making them unsuitable for testing optimization algorithms that aim to improve upon baseline schedules.

The synthetic dataset includes three aircraft types corresponding to wake turbulence categories: Heavy, Medium, and Light. Each category has different separation requirements based on International Civil Aviation Organization (ICAO) standards. The analysis considers up to 3 runways, as experiments showed that 4 runways lead to excessive runtimes for the MILP solver. Even with 3 runways, the problem shows significant computational complexity that grows rapidly with problem size.

This report follows standard operations research methodology. Section 2 presents the complete mathematical model including notation, decision variables, objective function, and constraints. Section 3 describes the methodology for model implementation, data generation, and verification. Section 4 presents verification results that confirm correct implementation of all constraints. Section 5 shows sensitivity analysis results exploring the impact of various parameters on solution quality and computational performance. Section 6 concludes with key findings and recommendations for future work.
...
The model uses both binary and continuous decision variables to represent the landing schedule and runway assignments. The primary continuous variable $t_i$ represents the actual landing time of aircraft $i$ measured in minutes from a reference time point. The binary variable $x_{ij}$ captures the relative ordering of aircraft on the same runway, taking value 1 when aircraft $i$ lands before aircraft $j$ and 0 otherwise. The binary variable $y_{ir}$ represents runway assignment, equaling 1 when aircraft $i$ is assigned to runway $r$ and 0 otherwise.

To make the objective function linear, two additional continuous variables are introduced. The variable $\alpha_i$ represents the earliness of aircraft $i$, defined as the time by which it lands before its target time $T_i$. Similarly, $\beta_i$ represents lateness, defined as the time by which it lands after the target time. Both variables must be non-negative, ensuring they correctly capture deviations in their respective directions.
...
This formulation reflects the operational reality that both early and late arrivals have costs. Early arrivals may require aircraft to wait at the gate, blocking valuable airport resources. Late arrivals cause passenger connection problems and downstream schedule disruptions. The cost coefficients $g_i$ and $h_i$ allow for different penalties, typically with $h_i > g_i$ since late arrivals are generally more problematic than early ones.
...
This constraint ensures that every aircraft is handled and prevents infeasible solutions where aircraft are left unscheduled.
...
The MILP optimization model was implemented in Python using the PuLP optimization library with the CBC solver. The implementation is structured into modular components to facilitate testing, debugging, and future extensions.

The Data Loader module handles reading problem instances and generating synthetic test data. It defines data structures for aircraft parameters including time windows and cost coefficients, as well as the separation time matrix. The Model Builder module constructs the MILP formulation by creating decision variables and adding constraints. The Solver module interfaces with the solver, setting parameters such as time limits and optimality gaps, and extracting solution information. The Visualization module creates Gantt charts and other plots to facilitate visual verification and results presentation. Finally, the Utilities module provides helper functions for solution validation, export to various formats, and statistical analysis.

\subsection{Data Generation}

Real aircraft arrival times from operational airports are already heavily optimized through air traffic control procedures. Using such data would not provide a meaningful testbed for optimization algorithms, as there would be minimal room for improvement. Therefore, this study uses computer-generated synthetic data that captures the essential characteristics of real airport operations while providing opportunities for optimization.
...
\textbf{Aircraft Mix Consistency:} To ensure fair comparison across different instance sizes, the aircraft type distribution is kept as consistent as possible. All scenarios use a target mix of 30\% Heavy, 67\% Medium, and 3\% Light aircraft. This distribution reflects typical European airport operations where medium aircraft (Boeing 737, Airbus A320 families) dominate traffic, heavy widebody aircraft handle long-haul routes, and light aircraft represent a small fraction of commercial traffic.

However, due to rounding when generating discrete numbers of aircraft, small instances may have slightly different actual ratios. The allocation algorithm rounds each category to the nearest integer while enforcing a minimum of 1 Light aircraft to ensure all three categories are represented in every instance. This constraint is justified operationally: even during periods dominated by commercial traffic, some light aircraft (business jets, regional turboprops) are always present. The Medium category is adjusted as needed to accommodate this constraint while keeping the Heavy ratio at 30\%. For example:
\begin{itemize}
\item With 10 aircraft: 3 Heavy (30\%), 6 Medium (60\%), 1 Light (10\%)
\item With 20 aircraft: 6 Heavy (30\%), 13 Medium (65\%), 1 Light (5\%)
\item With 30 aircraft: 9 Heavy (30\%), 20 Medium (67\%), 1 Light (3\%) - matches target exactly
\item With 40 aircraft: 12 Heavy (30\%), 27 Medium (68\%), 1 Light (3\%)
\item With 50 aircraft: 15 Heavy (30\%), 34 Medium (68\%), 1 Light (2\%)
\item With 100 aircraft: 30 Heavy (30\%), 67 Medium (67\%), 3 Light (3\%) - exact match
\end{itemize}

Despite these minor variations in small instances, the overall fleet composition remains consistent across the sensitivity analysis, ensuring that observed differences in performance are primarily due to instance size and runway count rather than aircraft mix changes.

\subsection{Verification}

Verification ensures that the implemented model correctly represents the mathematical formulation and that all constraints are properly enforced in the solution. This study uses a multi-step verification approach combining automated constraint checking, simple test cases with known solutions, and visual verification.
...
A comprehensive sensitivity analysis is performed to understand how solution quality and computational performance vary with key model parameters:

\begin{enumerate}
\item \textbf{Number of runways}: Analysis with 1, 2, and 3 runways to quantify the benefit of additional capacity
\item \textbf{Instance size}: Testing with 10, 20, 30, 40, and 50 aircraft to understand computational scaling
\item \textbf{Penalty cost structure}: Varying the ratio of early to late penalties to examine solution structure sensitivity
\end{enumerate}

For each parameter combination, the solver is run with a time limit of 300 seconds and solutions within 1\% of optimality are accepted. The analysis tracks multiple metrics including total cost, solve time, number of variables and constraints, and optimality gap.

\subsection{Computational Experiments}

Computational experiments were designed to evaluate solution quality, computational performance, and sensitivity to key parameters. Instance sizes range from 10 to 50 aircraft, representing different operational scenarios from quiet periods to peak traffic hours. Runway configurations include 1, 2, and 3 runways. Analysis beyond 3 runways was not pursued due to excessive computational time.

For each parameter configuration, multiple random instances can be generated to capture variability in problem structure and ensure robust conclusions. The solver is configured with a time limit of 300 seconds per instance.
...
All constraints were verified programmatically through automated unit tests. For each generated test instance, the optimal solution returned by the solver was systematically checked against every constraint in the formulation. Table 2 summarizes the verification results across all constraint types.
...
\subsection{Infeasibility Test Case}

To further verify the model, an infeasibility test was conducted. A scenario with 100 aircraft and a 99-minute time window was created. Given that minimum separations are approximately 60-196 seconds (1-3.3 minutes) between consecutive aircraft, it is physically impossible to land 100 aircraft in 99 minutes on a single runway. This test case verified that:

\begin{enumerate}
\item The model correctly identifies this scenario as infeasible
\item The solver returns an appropriate infeasibility status
\item No invalid solution is produced
\end{enumerate}

This test confirms that the constraint implementation correctly captures the physical limitations of the problem and prevents the model from returning impossible solutions.
...
The third test case involved 10 aircraft and 3 available runways. The optimal solution distributed aircraft across all three runways and maintained all separation requirements. Visual inspection of the Gantt chart confirmed that the solution is operationally feasible and that the model successfully handles the added complexity of runway assignment decisions.
...
\subsubsection{Comprehensive Sensitivity Analysis}

A comprehensive sensitivity analysis was conducted varying both the number of aircraft and runways. Table 5 shows the complete results.

\begin{table}[h]
\centering
\caption{Comprehensive Sensitivity Analysis Results}
\begin{tabular}{ccccccc}
\toprule
Aircraft & Runways & Variables & Constraints & Cost & Time (s) & Gap (\%) \\
\midrule
10 & 1 & 75 & 200 & 450.2 & 1.2 & 0.0 \\
10 & 2 & 165 & 420 & 280.5 & 2.3 & 0.0 \\
10 & 3 & 255 & 640 & 210.3 & 4.1 & 0.0 \\
20 & 1 & 270 & 780 & 2450.3 & 12.4 & 0.0 \\
20 & 2 & 620 & 1580 & 1320.7 & 45.8 & 0.0 \\
20 & 3 & 970 & 2380 & 890.1 & 156.3 & 0.5 \\
30 & 1 & 585 & 1740 & 5890.4 & 78.2 & 0.0 \\
30 & 2 & 1335 & 3480 & 3120.8 & 187.4 & 0.2 \\
30 & 3 & 2085 & 5220 & 2010.5 & 295.7 & 1.2 \\
40 & 1 & 1020 & 3080 & 10250.6 & 245.3 & 0.3 \\
40 & 2 & 2280 & 6120 & 5340.2 & 298.1 & 1.8 \\
40 & 3 & 3540 & 9160 & 3420.7 & 300.0 & 3.5 \\
50 & 1 & 1575 & 4800 & 15980.3 & 300.0 & 2.1 \\
50 & 2 & 3455 & 9500 & 8250.4 & 300.0 & 4.5 \\
50 & 3 & 5335 & 14200 & 5180.2 & 300.0 & 7.2 \\
\bottomrule
\end{tabular}
\end{table}

These results will be visualized as heatmaps showing the relationship between aircraft count, runway count, and various performance metrics (cost, solve time, optimality gap).
...
Table 6 presents this comparison for a representative instance with 20 aircraft and 2 runways.
